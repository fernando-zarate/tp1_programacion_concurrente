...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < SCHEDULER_QUANTITY; i++) {
            schedulers[i] = new Thread(new Scheduler(jobsContainer, jobsInQueue));
        }
        // Initialize PreExecutionCheck threads array.
        for (int i = 0; i < PRE_EXECUTION_CHECK_QUANTITY; i++) {
            preExecutionChecks[i] = new Thread(new PreExecutionCheck(jobsInQueue, jobsInExecution, jobsFailed));
        }
